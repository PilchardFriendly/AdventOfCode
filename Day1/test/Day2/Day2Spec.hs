...
spec = describe "Day2" $ do
  ...
  context "puzzle 2" $ do
    context "combine" $ allSamplesShouldBe
      combine
      [Raw ["a1", "b2", "c3"] [("a1", "b2"), ("a1", "c3"), ("b2", "c3")]]

    context "isSolution2" $ allSamplesShouldBe
      (uncurry isSolution2)
      [Raw ("abc", "") False, Raw ("abc", "abd") True]
  ...
  context "the input data" $ do
    let expectedLength = length expected
        expected :: [Text]
        expected =
    ...
    it "should have a solution 4 with" $ solve <$> parsedInput `shouldBe` Right
      7105

    it "should have neighbours" $ solve2a <$> parsedInput `shouldBe` Right
      (Just "omlvgdokxfncvqyersasjziup")
