...
module Day1.Day1Spec (spec) where
...
spec  = do
  ...
  describe "solve" $ 
    allSamplesShouldBe solve 
      [ Raw [1,2,3] 6
      , Raw [1,2] 3]

  describe "scanRepeating" $
    allSamplesShouldBe scanRepeating 
      [ Raw [1,2,3,1] [1]]

  describe "solving 2" $
    allSamplesShouldBe solve2
      [Raw [1,-1] $ Just 0
  ...
  describe "puzzleData" $ do
    ...
    it "should begin +13\\n+12" $ T.take 8 puzzleData `shouldBe` pack "+13\n-12\n"

    let 
      expected = Right [13,-12,-14, 19, -13]
      act = parseInput puzzleData
      select = (take 5 <$>)

    it "should parse Day1 data" $
        select act `shouldBe` expected
