...
spec = describe "Something" $ do
  let expectedDate = baseDate
      mkRange :: (FlexDateTime -> FlexDateTime) -> (FlexDateTime -> FlexDateTime) -> SleepRange
      mkRange start end = SleepRange $ (baseOffset $ baseDate & flexDT %~ start) +=+ (baseOffset $ baseDate & flexDT %~ end)
...
|]
  context "puzzle 1 " $ do
    let exampleShift = [SleepRange $ (5 * 60) +=+ (25 * 60)]
        sleep1min = SleepRange $ (5 * 60) +=+ (6 * 60)
    ...
    context "solve simple" $ do
      ...
      context "minutesForGuard" $
        allSamplesShouldBe
          (\(a, c, b) -> Map.lookup b $ minutesForGuard (toSolveable a) c)
    ...
    context "shift" $ do
      it "should parse" $
        parseOnly (shiftP mkSleep) simpleShift
          `shouldBe` Right (Shift baseDate 10 [SleepRange $ (5 * 60) +=+ (25 * 60)])
      it "should parseB" $
  ...
  context "example" $ do
    let parsedExample = parseOnly parseInput example
